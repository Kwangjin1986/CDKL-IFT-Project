...
##remove_osm-3
#turns_data <- turns_data[turns_data$strain == "n2" | turns_data$strain == "gcy-9" | turns_data$strain == "nx131" | turns_data$strain == "tm4182",]
...
## save data as a file
write.table(turns_data2, file="1peakfound.csv", sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE, append=FALSE)
...
## save data as a file
write.table(turns_by_interval2, file="2peaks_on_interval.csv", sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE, append=FALSE)
...
## save data as a file
write.table(good_number_by_interval3, file="3how_many_worms_on_plate.csv", sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE, append=FALSE)
...
## save data as a file
write.table(turns_min_agg, file="4summary.csv", sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE, append=FALSE)
...
ha_plot <- ggplot(turns_min_agg, aes(y = mean_tpm, x = interval, colour = strain)) + 
  ...
  geom_point() +
  labs(x="Time", y="Fraction High amplitude turns") 


#save figures
print("saving figure...")
ggsave(ha_plot, file="co2_omega-turn.pdf", h=4, w=6, units="in", dpi=300)
